...
Texas Hold'em itself, our initial gamemode, is one of the most 
...
The main engine of the backend is the evaluator, which takes all total cards on the table and evaluates the hand itself. 

\subsection*{Server}

Initially, an idea as defined in our Vision and Scope document was to use a UPnP / Peer to Peer approach, however this is infeasible as a key requirement of this project is disconnection-tolerance..
This is where we've opted for a client-server approach; as this allows atleast for one solid host, separate from the clients whom are likely to suffer disconnection issues. 

The servers design is such that on the start of the game, the server knows exactly what cards are going to be placed on the actual table, i.e. the initial flop (3 cards), turn and river albeit these will not be released to the player. It is responsible to send what cards each player has, and will utilize the previously mentioned Deck object to keep track of what cards have been used. 

The server is responsible for the game-state itself, which will prevent players becoming out of sync. 


\subsection*{Algorithms}

\entry{Hand Strength}
The algorithm for hand-strength is very basic and as such is ineffcient, however this is not the main focus of the project. We have methods which test for example, a straight (including straight flush), royal flush, a flush, etc. We then test each of these methods in order of ranking. \\
...
The example below shows how writing test cases becomes much easier through this method. Note, these are testing separate methods, but the meaning is still shown.
