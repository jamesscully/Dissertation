...
public class TexasEvaluator {
     ...
     * @return True if all com.scully.cards are the same suit
     */
    ...
    public TResult isRoyalFlush() {
        ...
        // we can only have a royal flush if there is 5> com.scully.cards of the same suit
        if( !(suitCountMap.containsValue(5) || suitCountMap.containsValue(6) || suitCountMap.containsValue(7)) )
            return null;

        Suit flush = null;

        // get the suit that contains the 5 com.scully.cards.
        ...
        for(Card c : cards) {
            // if it's not the suit that holds 5 of the com.scully.cards in the hand, continue
            if(c.getSuit() != flush)
                continue;
    ...
    public TResult isStraight() {
        ...
        System.out.println("Using com.scully.cards: " + cards);
